<script>
    ...
    Vue.component('stm-questions', {
        ...
        methods: {
            initComponent() {
                if (this.stored_ids) {
                    this.getPosts(stm_lms_ajaxurl + '?action=stm_curriculum&nonce=' + stm_lms_nonces['stm_curriculum'] + '&posts_per_page=-1&orderby=post__in&ids=' + this.stored_ids + '&post_types=' + this.posts.join(','), 'items');
                } else {
            ...
            createQuestion() {
                ...
                var url = stm_lms_ajaxurl + '?action=stm_curriculum_create_item&nonce='+ stm_lms_nonces['stm_curriculum_create_item'] + '&post_type=' + this.posts.join(',') + '&title=' + vm.add_new_question;
            ...
            changeTitle(post_id, title, itemKey) {
                if (isNaN(post_id)) {
                    ...
                    this.$http.get(stm_lms_ajaxurl + '?action=stm_save_title&nonce=' + stm_lms_nonces['action=stm_save_title'] + '&title=' + title + '&id=' + post_id);
                }
            },
            ...
            onSearch(search) {
                ...
                vm.searchTimeout = setTimeout(function () {
                    var url = stm_lms_ajaxurl + '?action=stm_curriculum&nonce=' + stm_lms_nonces['stm_curriculum'] + '&s=' + search + '&post_types=' + vm.posts.join(',');
                    vm.searching = true;
            ...
            saveQuestions() {
                ...
                this.$http.post(stm_lms_ajaxurl + '?action=stm_save_questions&nonce=' + stm_lms_nonces['stm_save_questions'], JSON.stringify(vm.items)).then(function () {
