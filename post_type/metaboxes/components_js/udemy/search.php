        ...
        methods: {
            search: function () {
                ...
                this.timer = setTimeout(function () {
                    _this.$http.get(stm_lms_ajaxurl + '?action=stm_lms_pro_search_courses&nonce=' + stm_lms_pro_nonces['stm_lms_pro_search_courses'] + '&s=' + _this.search_name).then(function (r) {
                        _this.loading = false;
            ...
            importCourse: function (course_id, index) {
                ...
                _this.$http.get(stm_lms_ajaxurl + '?action=stm_lms_pro_udemy_import_courses&nonce=' + stm_lms_pro_nonces['stm_lms_pro_udemy_import_courses'] + '&id=' + course_id).then(function (r) {
                    var r = r.body;

                    _this.$set(_this.courses[index], 'loading_text', r.message);
                    _this.$http.get(stm_lms_ajaxurl + '?action=stm_lms_pro_udemy_import_curriculum&nonce=' + stm_lms_pro_nonces['stm_lms_pro_udemy_import_curriculum'] + '&id=' + r.course_id).then(function (r) {
                        var r = r.body;
            ...
            publish: function (index, course_id) {
                ...
                _this.$http.get(stm_lms_ajaxurl + '?action=stm_lms_pro_udemy_publish_course&nonce=' + stm_lms_pro_nonces['stm_lms_pro_udemy_publish_course'] + '&id=' + course_id).then(function (r) {
