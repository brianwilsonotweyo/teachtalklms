<script>
    ...
    Vue.component('stm-manage-post-type', {
        ...
        methods: {
            getPosts() {
                var _this = this;
                var url = stm_lms_ajaxurl + '?action=stm_manage_posts&nonce=' + stm_lms_nonces['stm_manage_posts'];
            ...
            updateCourse(post) {
                ...
                _this.$http.get(stm_lms_ajaxurl + '?action=stm_lms_pro_udemy_import_courses&nonce=' + stm_lms_pro_nonces['stm_lms_pro_udemy_import_courses'] + '&id=' + post.id + '&update=true').then(function (r) {
                    var r = r.body;

                    _this.$set(post, 'loading_text', r.message);

                    _this.$http.get(stm_lms_ajaxurl + '?action=stm_lms_pro_udemy_import_curriculum&nonce=' + stm_lms_pro_nonces['stm_lms_pro_udemy_import_curriculum'] + '&id=' + r.course_id + '&update=true').then(function (r) {
                        var r = r.body;
            ...
            changeStatus(index, post_id, status) {
                ...
                _this.$http.get(stm_lms_ajaxurl + '?action=stm_lms_change_post_status&nonce=' + stm_lms_nonces['stm_lms_change_post_status'] + '&post_id=' + post_id + '&status=' + status).then(function (r) {
                    r = r.body;
